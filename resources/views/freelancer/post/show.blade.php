...
<div class="container-fluid">
    ...
    <a href="{{ route('freelancer.post.index') }}" class="btn btn-danger waves-effect">BACK</a>  
    ...
    @elseif($post->is_approved == true)
        ...
        @if($post->amount !== null)
        <a class="btn btn-default waves-effect">Project Earnings: <strong>${{ $post->earning }}</strong></a>
        @endif
    ...
    @if($post->is_completed!=true)
      <strong title="Time Left"><i class="material-icons">timer</i><span id="countdown"></span></strong>
    @endif    
...
<script src="{{ asset('assets/backend/js/jquery.countdown.js') }}"></script>
...
<script type="text/javascript">
  ...
  let date=deadline.substring(0,10).replace(/-/g, '/');
